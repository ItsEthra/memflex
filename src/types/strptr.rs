...
/// you may use [`Option<StrPtr>`] because this struct will benefit from non null optimizions.
...
impl StrPtr {
    ...
    /// Converts [`StrPtr`] to raw pointer.
    #[inline]
    pub fn as_ptr(&self) -> *const i8 {
        self.ptr.as_ptr() as _
    }

    /// Converts [`StrPtr`] to string slice.
    /// # Safety
    /// * [`StrPtr`] must point to a valid UTF-8 sequence.
    #[inline]
    pub unsafe fn as_str<'a>(&self) -> &'a str {
        core::str::from_utf8_unchecked(terminated_array::<u8>(self.ptr.as_ptr() as _, 0))
    }

    /// Converts [`StrPtr`] into signed byte slice.
    /// # Safety
    /// * [`StrPtr`] must be a valid pointer.
    #[inline]
